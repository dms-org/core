    ...
    public function testComplexUpdate()
    {
        ...
        $query = $this->platform->compileUpdate(
                Update::from($table)
                        ...
                        ->set('data', Expr::count())
                        ->join(Join::inner($table, 'bar', [Expr::equal(Expr::tableColumn($table, 'id'), Expr::column('bar', $table->getColumn('id')))]))
                        ->where(Expr::lessThan(Expr::tableColumn($table, 'id'), Expr::idParam(15)))
                        ->orderByDesc(Expr::tableColumn($table, 'id'))
                        ->offset(10)
                        ->limit(100)
    ...
    public function testComplexSelect()
    {
        ...
        $query = $this->platform->compileSelect(
                Select::allFrom($table)
                        ...
                        ->addHaving(Expr::greaterThan(Expr::column('t2', $table->getColumn('id')), Expr::idParam(10)))
                        ->orderByDesc(Expr::column('t2', $table->getColumn('id')))
                        ->offset(10)
                        ->limit(100)
        );
